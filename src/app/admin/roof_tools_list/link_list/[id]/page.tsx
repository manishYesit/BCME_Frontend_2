...
export default function roofToolList({ params }: any) {
  const id = params["id"];
  ...
  const handleDelete = (rowData: any) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          await axios.post(
            apiEndpoints.roofCodeTrashLinkId,
            { id: rowData.id },
            {
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const handleStatusUpdate = async (rowData: any, newStatus: number) => {
    try {
      ...
      const response = await axios.put(apiEndpoints.roofLinkStatus, payload, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let apiUrl = linkRowId
        ? apiEndpoints.roofCodeupdateLinkInformation
        : apiEndpoints.roofCodeAddLinkDataById;
