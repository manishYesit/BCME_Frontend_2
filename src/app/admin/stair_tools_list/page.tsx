...
// export default function stairToolsList() {

//     const [refresh, setRefresh] = useState(false);
//     interface StairTools {
//         sn: string,
//         title: string,
//         details: string,
//         link: string,
...
//     const columns = [
//         { name: "S.N", selector: (row: StairTools) => row.sn, sortable: true, width: "100px" },
//         { name: "Title", selector: (row: StairTools) => row.title, sortable: true, width: "200px", wrap: true },
//         { name: "Details", selector: (row: StairTools) => row.details, sortable: true, width: "500px", wrap: true },
//         { name: "Link", selector: (row: StairTools) => row.link, sortable: true, width: "100px" },
//         { name: "Actions", selector: (row: StairTools) => row.actions, sortable: true, width: "120px" }
//     ];
...
//         const result = await axios.get(apiEndpoints.getStairIconData, {
//             headers: {
...
//                     "title": obj.title,
//                     "details": obj.details
//                 }} updateUrl={apiEndpoints.updateStairIconData} deleteUrl={apiEndpoints.deleteStairIconData} token={token}/>
//         }
//         return templateObj;
//     });
...
//                                 <li className="breadcrumb-item active">Stair Tools</li>
//                             </ol>
//                         </div>
//                     </div>
//                     <div className="row mb-2">
//                         <div className="col-sm-6 float-sm-left" >
//                             <ModalComponent fields={fields} apiUrl={apiEndpoints.addStairIconData} token={token} title='Add stair tool'/>
//                         </div>
//                     </div>
//                 </div>
//             </section>
...
export default function StairToolList({}) {
  ...
  async function fetchData(token: any) {
    try {
      const { data } = await axios.get(apiEndpoints.getStairToolsData, {
  ...
  const columns = [
    ...
    { field: "details", header: "Data" },
    {
      ...
      body: (rowData: any) => (
        <>
          {rowData.id && (
            <Link href={`/admin/stair_tools_list/link_list/${rowData.id} `}>
              <p>Code</p>
            </Link>
          )}
        </>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let apiUrl = linkRowId
        ? apiEndpoints.updateStairToolsData
        : apiEndpoints.addStairToolsData;
  ...
  const handleDelete = (rowData: any) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          await axios.post(
            apiEndpoints.deleteStairToolsData,
            { id: rowData.id },
            {
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        <div className="container-fluid">
          ...
          >
            <ol className="breadcrumb float-sm-right">
              ...
              <li className="breadcrumb-item active">Stair Tools List</li>
            </ol>
          </div>
        </div>
      </section>
