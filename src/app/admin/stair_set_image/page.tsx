...
export default function StairSetOnImage() {
  ...
  async function UploadRoofImage(token: any, image: File) {
    try {
      ...
      const result = await axios.post(apiEndpoints.uploadStairImage, formData, {
        headers: {
  ...
  async function updateRoofCordinate(token: any) {
    try {
      ...
      const result = await axios.post(
        apiEndpoints.updateStairCordinate,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchImages = async (token: string) => {
      try {
        ...
        const response = await axios.get(apiEndpoints.getStairImage, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        const imageUrl =
          "http://3.12.198.252/api/assets/images/stair/" +
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let method = clickHammerData?.id ? axios.put : axios.post;
      ...
      toast.current?.show({
        severity: "success",
        detail: clickHammerData?.data?.id
